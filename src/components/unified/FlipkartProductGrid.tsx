import type { FlipkartProduct } from "../../types/unified";

type Props = {
  products: FlipkartProduct[];
  lastSearchQuery: string;
  onSelect: (p: FlipkartProduct) => void;
};
...
export default function FlipkartProductGrid({
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold mb-2">Flipkart results</h3>

      <div className="max-w-6xl">
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
          {products?.slice(0, 18).map((p: any, index: number) => {
            const key = (p.product_url || "fk") + index;
