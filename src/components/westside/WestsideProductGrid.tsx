import type { WestsideProduct } from "../../types/westside";

type Props = {
  products: WestsideProduct[];
  selectedProductUrl?: string | null;
  onSelect: (p: WestsideProduct) => void;
};

export default function WestsideProductGrid({
  products,
  selectedProductUrl,
  onSelect,
}: Props) {
  return (
    <div className="space-y-3">
      <h3 className="text-lg font-semibold mb-2">Westside products</h3>

      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
        {products.slice(0, 5).map((p, index) => {
          const key = `${p.product_url}-${index}`;
          const isSelected = selectedProductUrl === p.product_url;

          return (
            <div
              key={key}
              onClick={() => {
                console.log("WESTSIDE UI product clicked", p);
                onSelect(p);
              }}
              className={[
                "relative flex flex-col rounded-2xl overflow-hidden shadow-sm cursor-pointer transition-colors",
                isSelected ? "bg-[#1e1416] hover:bg-[#151622]" : "bg-[#11121a]",
              ].join(" ")}
            >
              <div className="relative w-full  bg-gray-800">
                {p.image_url ? (
                  <img
                    src={p.image_url}
                    alt={p.product_name}
                    className="w-full h-full object-contain"
                    onError={(e) => {
                      console.log(
                        "WESTSIDE UI image failed to load",
                        p.image_url
                      );
                      (e.currentTarget as HTMLImageElement).style.display =
                        "none";
                    }}
                  />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-xs text-gray-400">
                    No image
                  </div>
                )}
              </div>

              <div className="flex-1 flex flex-col px-3 py-3 gap-2">
                {/* <div className="min-h-[52px] space-y-1">
                  <p className="text-xs text-gray-300 line-clamp-2">
                    {p.product_name}
                  </p>
                </div>

                <div className="flex items-center justify-between mt-1">
                  <p className="text-base font-bold text-white">{p.price}</p>
                </div> */}

                <div className="mt-2 flex justify-center">
                  <button className="px-3 py-1.5 bg-red-600 hover:bg-red-500 text-white rounded-lg text-xs font-medium transition-colors">
                    Show details & size
                  </button>
                </div>
              </div>

              {isSelected ? (
                <div className="absolute bottom-2 right-12 w-6 h-6 rounded-full bg-red-600 flex items-center justify-center text-[10px] font-bold">
                  1
                </div>
              ) : null}
            </div>
          );
        })}
      </div>
    </div>
  );
}
