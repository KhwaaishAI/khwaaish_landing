...
export default function Chat1() {
  ...
  const handleOtpSubmit = async () => {
    ...
    try {
      const res = await fetch(`${BaseURL}api/submit-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Extract actual product list
      console.log(
        "STEP 03.5: Extracting actual product list from response.results"
      );

      const queryKey = Object.keys(data.results)[0];
      console.log("STEP 03.6: Query key =", queryKey);

      const productList = data.results[queryKey] || [];
  ...
  const handleConfirmCart = async () => {
    ...
    for (const [productName, qty] of selectedItems) {
      ...
      console.log("STEP 04.2: Add to cart API request sending...", payload);

      try {
        const res = await fetch(`${BaseURL}api/add-to-cart`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = await res.json();
        console.log("STEP 04.3: Add to cart API response:", data);
        ...
        if (data === null) {
          ...
          console.log("STEP 04.4: Address missing → show address popup");
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-4">
            ...
            <button
              ...
              className="w-full bg-red-600 hover:bg-red-500 text-white py-2 rounded-xl mt-4 font-semibold"
            >
              Confirm
            </button>
          </div>
