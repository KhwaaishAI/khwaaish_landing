...
export default function ZeptoAutoOrder() {
  const [oldUser, setOldUser] = useState(true);
  const [zeptoNew, setZeptoNew] = useState(false);
  ...
  async function handleSend() {
    ...
    try {
      const response = await fetch(
        `https://api.khwaaish.com/api/zepto-new/query`,
        {
          method: "POST",
          headers: {
      ...
      if (data.status === "need_login") {
        ...
        setIsLoading(false);
        pushSystem(
          "Order Placed Successfully, Please complete the payment on your UPI App."
        );
        return;
      } else {
        setIsLoading(false);
  ...
  async function handleLogin() {
    ...
    try {
      const response = await fetch(`https://api.khwaaish.com/api/zepto/login`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async function handleOtp() {
    ...
    try {
      const response = await fetch(
        zeptoNew
          ? `https://api.khwaaish.com/api/zepto-new/otp`
          : `https://api.khwaaish.com/api/zepto/enter-otp`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const data = await response.json();
      console.log("Data: ", data);
      if (data.status === "success" && zeptoNew) {
        pushSystem(
          "Order Placed Successfully, Please complete the payment on your UPI App."
        );
      }
  ...
  return (
    <>
      ...
      {oldUser && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[9999]">
          <div className="bg-gray-900 p-6 rounded-2xl w-100 space-y-4 border border-gray-700">
            <h2 className="text-xl font-semibold text-white">
              Is this your first time on Khwaaish?
            </h2>
