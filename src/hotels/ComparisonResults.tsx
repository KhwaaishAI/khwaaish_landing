...
const ComparisonResults: React.FC<ComparisonResultsProps> = ({
  ...
  const [sortBy, setSortBy] = React.useState<
    "cheapest" | "highest" | "rating" | "comparison"
  >("comparison");
  const [selectedPlatformByHotel, setSelectedPlatformByHotel] = React.useState<
    Record<string, "agoda" | "oyo" | "booking">
  >({});
      ...
      <div className="space-y-6">
        {allDisplayHotels.map((hotel, index) => {
          ...
          const selectedPlatform =
            selectedPlatformByHotel[hotelKey] || hotel.bestPrice.platform;
          ...
          return (
            <div
              key={index}
              className={`flex flex-col md:flex-row  bg-[#151515] rounded-2xl overflow-hidden shadow-lg border ${
                isComparison
                  ? "border-red-600/50 shadow-[0_0_10px_rgba(220,38,38,0.15)] scale-[1.01]"
                  : "border-gray-800"
              } hover:border-red-600 transition-all duration-300 h-auto md:h-64`}
            >
              {/* Left: Image */}
              ...
              {/* Middle: Details */}
              <div className="flex-1 p-5 hide-scrollbar flex flex-col h-full border-r border-gray-800 overflow-y-auto custom-scrollbar">
                <div className="mb-2">
                  <h3 className="text-xl font-bold text-white leading-tight hover:text-red-500 transition-colors cursor-pointer line-clamp-2">
                    {displayName}
                  </h3>
                ...
                <div className="mt-auto pt-2 border-t border-gray-800">
                  <div className="space-y-1">
                    {platforms.map((p, i) => {
                      const isBest = p.name === selectedPlatform;
                      const isCheapest = p.name === hotel.bestPrice.platform;

                      return (
                        <div
                          key={i}
                          className={`flex justify-between items-center text-xs py-2 px-3 rounded cursor-pointer transition-colors ${
                            isBest
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`font-medium capitalize truncate w-16 ${
                                isBest ? "text-white" : "text-gray-400"
                              }`}
                            >
                              {p.name}
                            </span>
                          ...
                          <span
                            className={`font-bold ${
                              isBest ? "text-red-400 text-sm" : "text-gray-400"
                            }`}
                          >
                            {p.price}
                            {isCheapest && (
              ...
              <div className="md:w-[220px] p-5 flex flex-col justify-center items-end bg-[#111] border-l border-gray-800 flex-shrink-0 relative">
                {isComparison && (
                  <div className="absolute top-0 right-0 left-0 bg-red-600/10 border-b border-red-600/30 text-center py-1">
                    <p className="text-[9px] font-bold text-red-400 tracking-widest uppercase">
                      Best Price Found
                    </p>
                  </div>
                )}
                ...
                <div className="text-right w-full mt-4">
                  ...
                  <div className="text-2xl font-bold">{activePrice}</div>

                  <p className="text-[10px] text-gray-600 mt-1 font-medium text-right mb-4">
                    + taxes & fees
                  </p>

                  <button
                    onClick={() => {
                      const bestP = platforms.find(
                        (p) => p.name === hotel.bestPrice.platform
                      );
                      if (bestP) {
                        const h =
                          selectedPlatform === "agoda"
